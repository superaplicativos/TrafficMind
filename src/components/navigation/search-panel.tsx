...
import { Search, X, MapPin, Loader2, Crosshair, CircleDot } from 'lucide-react';
 ...
 *   - origin status chip (shows whether GPS origin is set)
 */
export function SearchPanel({ onLocateMe, locating }: SearchPanelProps) {
  ...
  const pick = (r: GeocodingResult) => {
    ...
    setOpen(false);
    clear();
    toast.success('Destination set', { description: r.label });
  };
      ...
      <div className="glass-panel-strong rounded-2xl p-2 shadow-2xl">
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            size="icon"
            className={cn(
              'h-11 w-11 shrink-0 rounded-xl bg-secondary/60 hover:bg-secondary',
              !origin && !locating && 'ring-2 ring-primary ring-offset-2 ring-offset-background animate-pulse',
            )}
            onClick={onLocateMe}
            disabled={locating}
            aria-label="Locate me"
            title={origin ? `Origin: ${origin.lat.toFixed(4)}, ${origin.lng.toFixed(4)}` : 'Use my location'}
          >
            {locating ? <Loader2 className="size-4 animate-spin" /> : <Crosshair className="size-4" />}
          </Button>
        </div>

        {/* Origin status chip — tells the user whether the starting point is set */}
        <div className="mt-1.5 flex items-center gap-1.5 px-1 text-[11px]">
          {origin ? (
            <span className="flex items-center gap-1 text-emerald-400">
              <CircleDot className="size-3" />
              <span className="font-medium">Starting point set</span>
            ...
            <span className="flex items-center gap-1 text-amber-400">
              <CircleDot className="size-3" />
              <span className="font-medium">No starting point</span>
              <span className="text-muted-foreground">· tap the crosshair to set it</span>
            </span>
          )}
        </div>
      </div>
